...
torque, two-pole halving of the 865.5-d alias).
...
| observed CW (eopc04 x, 1962–2024, Hann FFT) | 0.8424 c/yr = 433.59 d |
| observed CW (eopc01 x, 1846–2024) | 0.8388 c/yr = 435.4 d |
| frequency gap, prediction vs modern obs | 0.0019 c/yr = 0.22% (phase drift 0.12 cyc over 62 yr) |
...
whose natural frequency is the canonical 433.85 d (0.8424 c/yr) must
drift through the carrier frame **monotonically** at −0.0019 c/yr
(−0.11 cyc over 58 yr, and that frame is generous to the carrier). It does not: the 16-yr slips **alternate sign**
...
Frequency-domain complement: the Hann-FFT peak of the monthly-cleaned
c04 record sits at 433.59 d, 0.22% off the predicted carrier — the
forced-only reading must eventually account for that offset. Candidate:
comb sidebands of the 2N line (0.844 ± 0.25/0.5/1 c/yr from the sampler
slots) pull the centroid; the nodal ± 0.0537-c/yr pair straddles the
line at ±2.4 d. And the carrier coherence has a time-domain summary:
...
- The 433.59-d observed vs 432.61-d predicted gap (0.22%) is real and
  unexplained here; the free-mode literature attributes the analogous
  offset to elastic-Earth loading. A forced reading must account for it
  (§3 candidates). The slip statistic is insensitive to it by
  construction — a constant offset is a linear phase term absorbed in
  the frame — so the headline is specifically *carrier-frame phase
  stationarity*, not frequency equality.

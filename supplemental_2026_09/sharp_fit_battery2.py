...
for idx, Ms in DESIGNS.items():
    ...
    r1r, cvr = stats(x)
    # ladder from THIS index's own spectrum + rho (per-index nulls)
    SS.x = x; SS.N = len(x); SS.freqs = np.fft.rfftfreq(len(x), d=1/12)
    SS.A0 = np.abs(np.fft.rfft(x))
    SS.f_lo = SS.freqs > 0
    fams = {}
    for alpha, name in ((0.0, "flat"), (1.0, "a1"), (2.0, "a2")):
        tilt = SS.tilt(alpha)
        fams[name] = [SS.iaaft_pass(100 + i, tilt) for i in range(8)]
    fams["ar1"] = [SS.ar1(float(r1r), 200 + i) for i in range(8)]
    ...
    for fam, vs in fams.items():
        vs2 = vs   # v2: ladder already built on this index's grid
...
json.dump(res, open(f"{ROOT}/supplemental_2026_09/figures/"
                    "sharp_battery_v2.json", "w"), indent=1)
print("\nsaved figures/sharp_battery_v2.json")
